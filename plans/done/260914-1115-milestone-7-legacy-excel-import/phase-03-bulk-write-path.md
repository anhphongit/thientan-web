...
**Status: Complete (2026-09-17). Test-copy validation and live write all successful.**

- [x] A full run against a **test copy** of the spreadsheet completes without exceptions,
      writes the expected number of `Orders`/`OrderLines`/`Invoices`/`StatusHistory` rows
      — *offline write tests pass; test-copy execution runbook Step 4 completed (2026-09-16)*
- [x] Spot-checking 10 random imported orders against their source Excel rows shows correct
      `po`, `customer`, `orderDate`, line amounts, VAT rate, and (where present) status
      — *runbook Step 5 manual validation completed (2026-09-16)*
- [x] `Orders.totalExVat`/`totalIncVat` match the sum of that order's `OrderLines` amounts
      exactly (server-side recompute, not trusted from the source file) — *offline tests pass; real execution confirmed (2026-09-17)*
- [x] Invoice linkage: an invoice number appearing on lines across multiple orders in the
      source file resolves to the **same** `Invoices.invoiceId` in all of them — *offline tests pass; real execution confirmed (2026-09-17)*
