...
- [ ] Offline test: a simulated capped run (e.g. 2 of 5 orders) followed by a second run
      completes the remaining 3 with zero duplicate `orderId`s and zero missing orders
- [ ] Manually killing a real run partway (e.g. via Apps Script's execution timeout on a small
      test copy) and re-running `migrateImportLegacyOrders()` completes cleanly with correct
      final counts
- [ ] `resetLegacyImportResumePoint()` exists and is documented in the Phase 5 runbook as an
      explicit, deliberate action (not something a confused re-run stumbles into)
