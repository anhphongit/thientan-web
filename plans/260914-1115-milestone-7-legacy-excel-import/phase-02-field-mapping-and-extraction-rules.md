...
- [ ] All mapping functions covered by offline tests using verbatim `EXCEL_REFERENCE.md`
      examples (PO 3-line split, VAT 1.08/1.10 ratios, all 5 controlled status values, at least
      2 deposit/supplier free-text examples, at least 2 UoM casing variants)
- [ ] Running `mapLegacyOrderGroup_` over every group from Phase 1's real dry-run parse produces
      zero uncaught exceptions (a messy/unexpected row degrades to a review flag, never a crash)
- [ ] Review-flag counts (VAT fallback, status unmatched, UoM no-match, deposit-extraction
      attempted) are printed in an updated dry-run report and are small enough to spot-check by
      hand (expect low tens, not hundreds, given `EXCEL_REFERENCE.md`'s counts)
